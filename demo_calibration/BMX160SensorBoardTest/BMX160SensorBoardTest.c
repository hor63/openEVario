...
static int openSocket() {
	...
	if (prot != NULL) {
		...
		addrHint.ai_protocol = 6;
	}

	addrHint.ai_family = AF_INET;
	addrHint.ai_socktype = SOCK_STREAM;

	printf ("Addr = \"%s\", port = \"%s\"\n",MAKE_IP_ADDR_I2C_BRIDGE,MAKE_PORT_STR(PORT_I2C_BRIDGE));

	rc = getaddrinfo(MAKE_IP_ADDR_I2C_BRIDGE,MAKE_PORT_STR(PORT_I2C_BRIDGE),&addrHint,&addr);
	...
	if (sock != -1) {
		...
		if (rc == -1) {
			rc = errno;
			fprintf(stderr,"setsockopt TCP_QUICKACK error: %s\n",strerror(rc));
		}
...
*/
	...
	n = read (sock,&data.header,sizeof(data.header));
	while (n ==  sizeof(data.header)) {
		...
		case BMX160DATA_TRIM:
			if (data.header.length == (sizeof(data.header)+sizeof(data.trimData))) {
				if (read(sock,&data.trimData,sizeof(data.trimData)) == sizeof(data.trimData)) {
		...
		case BMX160DATA_ACC_GYR_MAG:
			if (data.header.length == (sizeof(data.header)+sizeof(data.accGyrMagData))) {
				if (read(sock,&data.accGyrMagData,sizeof(data.accGyrMagData)) == sizeof(data.accGyrMagData)) {
